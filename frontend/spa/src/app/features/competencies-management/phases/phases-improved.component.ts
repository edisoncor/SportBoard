...
export class PhasesComponent implements OnInit, AfterViewInit {
  ...
  loadPhases(): void {
    this.loading = true;
    // Simulación de carga de fases
    setTimeout(() => {
      this.phases = this.mockPhases;
    ...
    if (this.phaseForm.valid) {
      ...
      setTimeout(() => {
        if (this.editingPhase) {
          ...
          const index = this.phases.findIndex(p => p.id === this.editingPhase!.id);
          if (index !== -1) {
            this.phases[index] = { ...this.editingPhase, ...phaseData };
            this.dataSource.data = this.phases;
          }
          this.showSuccess('Fase actualizada exitosamente');
        } else {
          // Crear nueva fase
          const newPhase: PhaseDisplay = {
            id: this.phases.length + 1,
            name: phaseData.name,
            description: phaseData.description,
            modality: phaseData.modality || 1,
            season: phaseData.season || 1,
            category: phaseData.category || 1,
            isActive: true,
            ...phaseData
          };
