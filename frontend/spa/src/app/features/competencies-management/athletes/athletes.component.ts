...
import { CompetencyUser } from '../../../core/models/competencies/User';
            ...
            <div class="form-section">
              ...
              <div class="form-row">
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  <mat-error *ngIf="athleteForm.get('height')?.hasError('min')">
                    La altura mínima es 100 cm
                  </mat-error>
                  <mat-error *ngIf="athleteForm.get('height')?.hasError('max')">
                ...
                <mat-form-field appearance="outline" class="form-field">
                  ...
                  <mat-error *ngIf="athleteForm.get('weight')?.hasError('min')">
                    El peso mínimo es 30 kg
                  </mat-error>
                  <mat-error *ngIf="athleteForm.get('weight')?.hasError('max')">
                    El peso máximo es 200 kg
                  </mat-error>
              ...
              <ng-container matColumnDef="name">
                ...
                <td mat-cell *matCellDef="let athlete">
                  <div class="athlete-info">
                    ...
                    <div class="athlete-date">Atleta #{{ athlete.id }}</div>
                  </div>
                </td>
              </ng-container>
...
export class AthletesComponent implements OnInit {
  ...
  onSubmit(): void {
    if (this.athleteForm.valid) {
      ...
      if (this.editingAthlete) {
        this.updateAthlete(transformedData);
      } else {
        this.createAthlete(transformedData);
      }
    }
  }
  ...
  private transformFormData(formData: any): any {
    ...
    // Convert user ID to full URL format  
    if (transformed.user) {
      transformed.user = `http://localhost:8010/api/v1/competencies/users/${transformed.user}/`;
    }
    
    // Convert position to catalogue URL format
    if (transformed.position) {
      // Map position names to catalogue IDs
      ...
      const catalogueId = positionMapping[transformed.position.toLowerCase()] || parseInt(transformed.position) || 1;
      transformed.position = `http://localhost:8010/api/v1/competencies/catalogues/${catalogueId}/`;
    }
    
    // Ensure numeric fields are properly typed
    if (transformed.height) {
      transformed.height = parseFloat(transformed.height);
    }
    if (transformed.weight) {
      transformed.weight = parseFloat(transformed.weight);
    }
    
    console.log('Form data transformed:', transformed);
    return transformed;
  }
  ...
  editAthlete(athlete: Athlete): void {
    ...
    // Extract IDs from URLs for form controls
    const userId = this.extractUserIdFromAthlete(athlete.user);
    const positionId = this.extractIdFromUrl(athlete.position);
  ...
  private extractUserIdFromAthlete(user: number | CompetencyUser | any): string {
    if (typeof user === 'number') {
      return user.toString();
    } else if (typeof user === 'string') {
      // Extract ID from URL like "http://localhost/api/v1/competencies/users/1/"
  ...
  private extractIdFromUrl(url: string): string {
    // Extract ID from URL like "http://localhost/api/v1/competencies/catalogues/1/"
