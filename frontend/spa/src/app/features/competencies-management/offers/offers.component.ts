...
export class OffersComponent implements OnInit, AfterViewInit {
  ...
  loadOffers(): void {
    this.loading = true;
    this.competenciesService.getOffers().subscribe({
      next: (offers) => {
        this.offers = offers;
  ...
  saveOffer(): void {
    if (this.offerForm.valid) {
      ...
      if (this.editingOffer && this.editingOffer.id) {
        ...
        this.competenciesService.updateOffer(this.editingOffer.id, offerData).subscribe({
          next: (updatedOffer) => {
        ...
        // Crear nueva oferta
        this.competenciesService.createOffer(offerData).subscribe({
          next: (newOffer) => {
  ...
  deleteOffer(offer: OfferDisplay): void {
    if (confirm(`¿Está seguro que desea eliminar la oferta "${offer.name}"?`)) {
      if (offer.id) {
        this.competenciesService.deleteOffer(offer.id).subscribe({
          next: () => {
